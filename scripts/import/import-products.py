...
def extract_romana(ws) -> list[ProductRow]:
    ...
    """
    rows: list[ProductRow] = []
    modelos = [(7, "Tradicional", "TRA"), (8, "Cascade", "CAS"),
               (9, "Teto Bastão", "TEB"), (10, "Teto Monocorrente", "TEM"),
               (11, "Sky Light", "SKY")]
...
def extract_tela_mosquiteira(ws) -> list[ProductRow]:
    """Tela Mosquiteira — telas (m²), perfis (ml) e opcionais (pç).

    Layout: col A = numeração (1, 2, ...), col B = nome do item,
    col I = valor, col J = unidade.
    """
    rows: list[ProductRow] = []
    for r in range(5, ws.max_row + 1):
        b = ws.cell(row=r, column=2).value
        ...
        valor = br_num(ws.cell(row=r, column=9).value)
        unidade = ws.cell(row=r, column=10).value
        # Item de produto: col A numérico, col B com texto, valor numérico
        if not isinstance(b, str) or not isinstance(c, (int, float)):
            continue
        if valor is None or valor <= 0:
            continue
        # Determina secao baseado na proximidade de headers conhecidos
        # Heurística simples: se nome contém "Perfil" ou "Tela", ajusta produto
        b_strip = b.strip()
        if "Perfil" in b_strip:
            secao = "Perfis"
            metodo = "ml" if (unidade and "ml" in str(unidade).lower()) else "m2"
        elif "Travas" in b_strip or "Ângulo" in b_strip or "Recorte" in b_strip:
            secao = "Opcionais"
            metodo = "pç"
        else:
            secao = "Telas"
            metodo = "m2" if (unidade and "m" in str(unidade).lower() and "ml" not in str(unidade).lower()) else "ml"
        # Tenta extrair codigo do nome
        codigo_extraido = extract_cod(b_strip)
        if codigo_extraido:
            codigo = f"TM-{codigo_extraido}"
        else:
            slug = re.sub(r"[^A-Za-z0-9]+", "", b_strip).upper()[:6]
            codigo = f"TM-{c:03d}-{slug}"
        rows.append(ProductRow(
            nome=b_strip,
            codigo=codigo,
            produto="Tela Mosquiteira",
            modelo=secao,
            tecido=b_strip,
            preco_venda=valor,
...
def extract_vert_wave(ws) -> list[ProductRow]:
    """Vert Wave e Varão Ilhós 28 — 2 preços por tecido (Wave + Varão). Por metro linear."""
    rows: list[ProductRow] = []
    for r in range(6, ws.max_row + 1):
        b = ws.cell(row=r, column=2).value
        c = ws.cell(row=r, column=1).value
        wave = br_num(ws.cell(row=r, column=7).value)
        varao = br_num(ws.cell(row=r, column=10).value)
        largura = br_num(ws.cell(row=r, column=5).value)
        if not isinstance(b, str) or not isinstance(c, (int, float)):
            continue
        nome_base = b.strip()
        for valor, modelo, suf in [(wave, "Vertical Wave", "VW"), (varao, "Varão Ilhós 28", "VI")]:
            if valor is None or valor <= 0:
                continue
            rows.append(ProductRow(
                nome=f"{nome_base} {modelo}",
                codigo=f"VW-{c:03d}-{suf}",
                produto="Cortina Vertical Wave",
                modelo=modelo,
                tecido=nome_base,
                preco_venda=valor,
                metodo_calculo="ml",
...
def extract_toldos_tvx(ws) -> list[ProductRow]:
    """Toldos e Cort Vertical TVX 200 — matriz altura × largura por tecido.

    Para cada tecido (secao), gera 1 produto por tecido usando o preço base
    (1.2m × 1.50m = célula menor) e anota dimensões da matriz em `notes`.
    """
    rows: list[ProductRow] = []
    current_section = "Tecido"
    counter = 0
    # Os nomes de secao estao em linhas onde col B tem texto e nao ha valores
    for r in range(5, ws.max_row + 1):
        b = ws.cell(row=r, column=2).value
        c = ws.cell(row=r, column=1).value
        # detecta nome de secao (ex: 'PVC / PVC CRISTAL', 'SCREEN SOLFLEX 5%')
        if isinstance(b, str) and b.strip() and not isinstance(c, (int, float)) \
                and not br_num(ws.cell(row=r, column=3).value) \
                and not br_num(ws.cell(row=r, column=4).value):
            # Apenas se for nome curto (sem "ALTURA", etc)
            t = b.strip()
            if t.upper() not in ("ALTURA",) and not t.startswith("LARGURA"):
                current_section = t
                continue
        # Tecido: linha com col B = texto de tecido, col C vazio, col D+ preços
        # Os tecidos estao listados como secao; precos sao por altura×largura
        # Esta sheet é difícil — para evitar lixo, pego apenas se houver
        # padrao claro de tecido individual (geralmente nao tem por linha).
    return rows


def extract_demais_cortinas(ws) -> list[ProductRow]:
    """Demais Cortinas — tecidos com 9 preços (Forro, Cortina, Soft Wave 5x2/5x5/5x7.5,
    Franzida Mini/Normal/Colmeia, Prega Americana/Macho). Por metro linear."""
    rows: list[ProductRow] = []
    # coluna: modelo, valor
    modelos = [
        (5, "Forro", "FO"), (6, "Cortina", "CT"),
        (7, "Soft Wave 5x2", "SW52"), (8, "Soft Wave 5x5", "SW55"),
        (9, "Soft Wave 5x7.5", "SW575"), (10, "Soft Wave 3.5x5", "SW35"),
        (11, "Franzida Mini", "FZM"), (12, "Franzida Normal", "FZN"),
        (13, "Franzida Colmeia", "FZC"), (14, "Prega Americana", "PGA"),
        (15, "Prega Macho", "PGM"),
    ]
    for r in range(14, ws.max_row + 1):
        ...
        for col, modelo, suf in modelos:
            ...
            rows.append(ProductRow(
                nome=f"{nome} {modelo}",
                codigo=f"DC-{c:03d}-{suf}",
                produto="Demais Cortinas",
                modelo=modelo,
                tecido=nome,
                preco_venda=valor,
                metodo_calculo="ml",
...
SHEET_EXTRACTORS: dict[str, Callable] = {
    ...
    "Tela Mosquiteira": (extract_tela_mosquiteira, None),
    "Vert Wave e Varão Ilhós 28": (extract_vert_wave, None),
    "Demais Cortinas": (extract_demais_cortinas, None),
    # Toldos TVX: layout matriz altura×largura, sem linhas de tecido claras.
    # Precisa decisão manual.
}
